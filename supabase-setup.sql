...
-- EXECUTIONS (payment validation registry)
create table if not exists public.executions (
  id               uuid primary key default gen_random_uuid(),
  execution_id     text unique not null,
  status           text not null,  -- APPROVED / REQUIRES_REVIEW / BLOCKED
  authorized       boolean default false,
  hold_pending     boolean default false,
  budget           bigint,
  timeline         integer,
  complexity       integer,
  risk_score       numeric(5,2),
  loss_exposure    bigint,
  reason           text,
  entity           text,
  country          text,
  payment_type     text,
...
create index if not exists idx_executions_execution_id
  on public.executions(execution_id);
