/**
 * ENTRY fully-ready acceptance — production workspace only.
 * Targets: / /entry → root index.html + Entry assets (not WEB-001 package).
 */
...
test('production ENTRY uses Execution Standard section order', async () => {
  ...
  assert.ok(html.includes('entry-landing.css'), 'ENTRY landing styles required');
...
test('production ENTRY declares the Execution Standard category', async () => {
  ...
  assert.ok(/For People/i.test(html), 'people audience missing');
  ...
  assert.ok(
    html.includes('Review the EXECUTIA Engine') ||
      html.includes('Review the Engine') ||
      html.includes('Experience the Engine'),
  ...
  assert.ok(
    html.includes('Available for review') || html.includes('Available to experience'),
    'LIFE maturity signal missing'
  );
  ...
  assert.ok(/better execution|after a decision/i.test(html), 'central execution idea missing');
  assert.ok(/creat|learn|collaborat|liv/i.test(html), 'Vision human frame missing');
  assert.ok(
    /under development|in development|being built|working toward|validat/i.test(html),
  ...
  assert.equal(html.includes('not a sales process'), false, 'defensive sales language must not ship');
...
test('production ENTRY navigation has Request Pilot and Execution Standard links', async () => {
  ...
  assert.ok(nav.includes('/#platform'), 'Platform anchor required');
  assert.ok(nav.includes('/#gov'), 'GOV anchor required');
  assert.ok(nav.includes('/#development'), 'Development anchor required');
...
test('production ENTRY accessibility: skip-link, main landmark, reduced motion', async () => {
  ...
  const entryCss = await load('assets/entry-landing.css');
  assert.ok(appCss.includes('.skip-link') || entryCss.includes('.skip-link'), 'skip-link styles missing');
  assert.ok(
    entryCss.includes('prefers-reduced-motion') || appCss.includes('prefers-reduced-motion'),
