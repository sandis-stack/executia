    ...
    <div style="font-family:Inter,Arial,sans-serif;background:#f8fafc;padding:32px;">
      <div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #DEE2E6;padding:32px;">
        ...
        <div style="border-left:3px solid #0A1F44;padding:16px 18px;background:#F5F7FA;margin-bottom:24px;font-family:'JetBrains Mono',monospace;font-size:11px;line-height:2;">
          ${data.executionId ? `EXECUTION ID: ${data.executionId}<br>` : ''}
...
function buildInternalEmailHtml(data, leadId, req) {
  ...
  return `
    <div style="font-family:Inter,Arial,sans-serif;line-height:1.6;color:#0A2540;">
      ...
      ${data.executionId ? `<p><strong>Execution ID:</strong> ${data.executionId}</p>` : ''}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const isExecution = !!data.executionId || !!data.entity;
    ...
    const leadPayload = {
      ...
      ...(data.executionId ? { execution_id: data.executionId } : {}),
    };
    ...
    await insertLeadActivity(leadId, 'LEAD_CREATED', { source: data.source, executionId: data.executionId });
    ...
    try {
      clientResult = await resend.emails.send({
        ...
        subject: `EXECUTIA — Request received${data.executionId ? ' (' + data.executionId + ')' : ''}`,
    ...
    try {
      internalResult = await resend.emails.send({
        ...
        html:     buildInternalEmailHtml(data, leadId, req),
      });
    ...
    return json(res, 200, {
      ...
      client_email_sent: !!clientResult, internal_email_sent: !!internalResult,
