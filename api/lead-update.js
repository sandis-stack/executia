...
export default async function handler(req, res) {
  ...
  try {
    ...
    const byId = await supabase.from('leads').select('id,status,note').eq('id', leadId).maybeSingle();
    if (byId.data) {
      existingLead = byId.data;
    } else {
      const byEid = await supabase.from('leads').select('id,status,note').eq('execution_id', leadId).maybeSingle();
